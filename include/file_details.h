...
namespace myq {
  ...
  public:
       ...
       * @return
       */
      ssize_t read_msg(char *buffer, uint32_t buf_size, uint64_t offset, bool ntohl_flag) {
          LOG_IN("%s", utils::format_str(
              "buffer: %p, buf_Size: %d, offset: %llu ntohl_flag: %d", buffer, buf_size, offset,
              ntohl_flag).c_str());

          unsigned size_of_int = sizeof(unsigned);
          ssize_t size = read_buffer_length(buffer, buf_size, offset, ntohl_flag);
          if (size == 0) {
          ...
          if (size > buf_size) {
              LOG_ERROR("message size: %d  is larger than buffer size. Dynamic memory allocation not supported",
                        size);
     ...
     /*
      ssize_t splice_to_file(int socket) {
          LOG_IN("socket[%d]", socket);
         ...
         int pfd[2];
            ...
            pipe(pfd);
            for (;;) {
               ssize_t bytes = splice(socket, NULL, pfd[1], NULL, sizeof (buffer), SPLICE_F_MOVE|SPLICE_F_NONBLOCK);
                if (bytes == 0)
                    break;
       ...
       * @param msg_len
       * @param include_size
       * @param include_offset
       * @return
       */
      ssize_t write_msg(const char *msg, unsigned msg_len, bool include_size = true, bool include_offset = true) {
          LOG_IN("msg:[%p], msg_len[%u], include_size[%d], include_offset[%d]",
                 msg, msg_len, include_size, include_offset);
          ssize_t length_written = 0;
          if (include_size) {
          ...
          size_t written_buffer = 0;
          if ((written_buffer = write_buffer(msg, msg_len)) <= 0) {
              LOG_RET("Error", written_buffer);
          }
  ...
  private:
       ...
       * @return
       */
      ssize_t read_buffer_length(char *buffer, int buf_size, uint64_t offset, bool ntohl_flag) {
          LOG_IN("buffer: %p, buf_size: %d, offset: %llu, ntohl_flag: %d", buffer, buf_size, offset, ntohl_flag);
          ssize_t result = 0;
          ...
          while (bytestoRead > 0) {
              ...
              if (result < 1) {
                  ...
                  LOG_ERROR("offset_across_all_files_[%llu], file offset[%llu]", bytes_written_across_all_files_,
                            offset_);
                  LOG_ERROR("buffer: %p, buf_size: %d, offset: %llu, ntohl_flag: %d", buffer, buf_size, offset, ntohl_flag);
                  LOG_RET("Error:", -1);
              }
              bytestoRead -= result;
              bytesRead += result;
