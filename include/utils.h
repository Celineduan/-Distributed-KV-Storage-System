...
namespace mymq
{
    ...
    public:
        ...
        inline static std::string format_str(const char *buf, ...)
        {
            va_list args;
            va_start(args, buf);
            std::string msg = myq::log::format_arg_list(buf, args);
            va_end(args);
            return msg;
        }
         ...
         * @return
         */
        static ssize_t read_size(int fd, bool ntohl_flag = false)
        {
            LOG_IN("fd[%d], ntohl_flag[%d]", fd, ntohl_flag);
         ...
         * @return
         */
        static ssize_t write_size(int fd, uint32_t length, bool htonl_flag = false)
        {
            LOG_IN("fd[%d], length[%u], htonl[%d]", fd, length, htonl_flag);
            if (htonl_flag)
            {
                length = htonl(length); // not needed for file io
            }
